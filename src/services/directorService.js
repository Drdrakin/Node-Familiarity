...
async function validateDirector(director){
    const sql = "select * from tbl_diretor where nome_diretor = ?"

    const conn = await database.connect();
    const [rows] = await conn.query(sql, director);
    conn.end();

    return rows;
}

export default {createDirector, updateDirector, listDirector, listDeletedDirector, softDeleteDirector, hardDeleteDirector, listSpecificDirector, validateDirector} 
